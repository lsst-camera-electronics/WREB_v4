...
library common;
use common.WREB_v4_commands_package.all;
...
entity wreb_v4_cmd_interpreter is
  ...
  port (
    ...
    image_pattern_read : in    std_logic_vector(0 downto 0);  -- this register gives the state of image patter gen. 1 is ON
    ccd_sel_read       : in    std_logic_vector(2 downto 0);  -- this register contains the CCD to drive
    ccd_oe_en          : out   std_logic;                     -- register enable for CCD output enable selector
    ccd_oe_read        : in    std_logic_vector(0 downto 0);  -- this register contains the CCD to connect to SCI
...
end entity wreb_v4_cmd_interpreter;
...
begin
  ...
  process (pres_state, regReq, regOP, RegAddr, regdatawr_masked, regwren, switch_addr, time_base_actual_value,
    ...
    -- CCD bias
           c_bias_dac_cmd_err, c_bias_v_undr_th, bias_gd_thresh, bias_od_thresh, bias_rd_thresh,
    -- Bacbias switch
           back_bias_sw_rb, back_bias_cl_rb, back_bias_sw_error,
           mgt_avcc_ok,
           sync_cmd_delay_read,
           interrupt_mask_read,
           image_size, image_pattern_read, ccd_sel_read, statusreg, seq_time_mem_readbk, ccd_oe_read,
           seq_out_mem_readbk, seq_prog_mem_readbk, enable_conv_shift_in, start_add_prog_mem_rbk, seq_ind_func_mem_rdbk,
           seq_ind_rep_mem_rdbk, seq_ind_sub_add_mem_rdbk, seq_ind_sub_rep_mem_rdbk, seq_op_code_error, seq_override_rd,
           v3_3v_ok,
           ...
           reb_sn_crc_ok, reb_sn, reb_sn_timeout, ccd1_clk_en_in, ccd2_clk_en_in,
           ...
           t2_reb_gr2, t3_reb_gr2, t4_reb_gr2, t1_reb_gr3, reb_sn_dev_error,
  ...
  begin
    ...
    case pres_state is
      ...
      when wait_cmd =>
        ...
        if (regReq = '0') then
          ...
          if (regOp = '0') then
            ...
            elsif (regAddr = image_pattern_mode_cmd) then
              next_state <= read_image_pattern_mode_state;
            ...
            -- reads indirect functions memory
                   ...
                   ((regAddr >= seq_ind_func_mem_base_0) and (regAddr <= seq_ind_func_mem_high_1))) then
      ...
      when read_image_pattern_mode_state =>

        next_state     <= wait_end_cmd;
        next_regAck    <= '1';
        next_regDataRd <= x"0000000" & "000" & image_pattern_read;
      ...
      when read_ccd_oe_state =>
        ...
        next_state     <= wait_end_cmd;
        next_regAck    <= '1';
        next_regDataRd <= x"0000000" & '0' & '0' & '0' & ccd_oe_read;
      ...
      -- enables the image pattern generator
